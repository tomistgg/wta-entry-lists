...
def main():
    ...
    for week, tournaments in TOURNAMENT_GROUPS.items():
        ...
        for url, label in tournaments.items():
            ...
            if has_new_data:
                ...
                <div class="top-row">
                    <div class="header-controls" style="display:flex; flex-direction:column; gap:6px;">
    ...
    <head>
        ...
        <style>
            ...
            .top-row {{ display: flex; align-items: center; justify-content: space-between; margin-top: 5px; margin-bottom: 20px; }}
            .header-controls {{ flex: 1; min-height: 80px; }}
            .spacer {{ flex: 1; }}
            ...
            .toggle-btn {{ background: rgba(255, 255, 255, 0.15); border: 1px solid white; color: white; padding: 5px 10px; border-radius: 20px; cursor: pointer; font-size: 0.68rem; font-family: 'MontserratSemiBold', sans-serif; backdrop-filter: blur(5px); max-width: 170px; width: 100%; text-align: center; }}
            ...
            @media (max-width: 768px) {{
                ...
                .header-controls {{ order: 3; width: 100%; min-height: auto; }}
                .toggle-btn {{ max-width: 100%; padding: 10px; }}
                .spacer {{ display: none; }}
    ...
    <body>
        ...
        function toggleView(btn) {{
            ...
            const changeView = activeTab.querySelector('.changes-view');
            const subTitle = activeTab.querySelector('.sub-title');
            const changesBtn = activeTab.querySelector('.changes-btn');
            const tempBtn = activeTab.querySelector('.temp-qual-btn');
            
            if (changeView.style.display === "flex") {{
                if (btn.innerText.includes("Qualifying")) {{
                    mainView.style.display = "none";
                    qualView.style.display = "flex";
                    activeTab.querySelector('.toggle-btn').innerText = "Switch to Main Draw";
                    subTitle.innerText = "QUALIFYING ENTRY LIST";
                }} else {{
                    mainView.style.display = "flex";
                    qualView.style.display = "none";
                    activeTab.querySelector('.toggle-btn').innerText = "Switch to Qualifying";
                    subTitle.innerText = "MAIN DRAW ENTRY LIST";
                }}
        ...
        function showChanges(btn, tid) {{
            ...
            const toggleBtn = activeTab.querySelector('.toggle-btn');
            const controls = activeTab.querySelector('.header-controls');
            ...
            toggleBtn.innerText = "Switch to Main Draw";
            
            if (!activeTab.querySelector('.temp-qual-btn')) {{
                const qualBtn = document.createElement("button");
                qualBtn.className = "toggle-btn temp-qual-btn";
                qualBtn.style.marginTop = "6px";
                qualBtn.innerText = "Switch to Qualifying";
                qualBtn.onclick = function() {{ toggleView(this); }};
                controls.appendChild(qualBtn);
            }}
        }}
        </script>
    </body>
