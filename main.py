...
def main():
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(f'<!DOCTYPE html><html><head>...</head><body><div class="sidebar">{sidebar_html}</div><div class="main-content">{content_html}</div><script>...</script></body></html>')
    ...
    for tid, entries in history.items():
        ...
        if current_run_changes:
            ...
            all_new_changes.append(f"Tournament: {tid}\n" + "\n".join(f"- {c}" for c in clean_changes))
