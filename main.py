...
<head>
    ...
    <style>
        ...
        .sidebar {{ width: 250px; background-image: url('FondoDegradado.png'); background-size: cover; border-right: 2px solid #ffffff; overflow-y: auto; padding: 10px; flex-shrink: 0; }}
        .week-title {{ font-family: 'MontserratExtraBold'; padding: 25px 10px 5px; font-size: 0.9rem; text-transform: uppercase; }}
        .tablinks {{ width: 100%; border: none; background: none; text-align: left; padding: 8px 10px; cursor: pointer; color: white; font-size: 0.8rem; }}
        ...
        .top-row {{ display: flex; align-items: center; justify-content: space-between; height: 80px; }}
        .header-controls {{ display: flex; flex-direction: column; gap: 6px; }}
        .title-stack {{ text-align: center; flex: 2; }}
        .sub-title {{ font-family: 'MontserratExtraBold'; font-size: 1.05rem; letter-spacing: 1.5px; }}
        .main-title {{ font-family: 'MontserratExtraBold'; font-size: 1.4rem; margin: 0; }}
        ...
        .tables-row {{ display: flex; gap: 20px; justify-content: center; }}
        .table-column {{ flex: 1; max-width: 550px; border: 1px solid rgba(255,255,255,0.35); border-radius: 6px; }}
        .entry-table {{ width: 100%; border-collapse: collapse; }}
        .entry-table th {{ background: rgba(255,255,255,0.1); padding: 10px; border-bottom: 1px solid rgba(255,255,255,0.25); }}
        .entry-table td {{ padding: 7px; border-bottom: 1px solid rgba(255,255,255,0.12); text-align: center; }}
        ...
        .spacer {{ flex: 1; }}
        @media (max-width: 768px) {{ body {{ flex-direction: column; }} .sidebar {{ width: 100%; height: auto; display: flex; overflow-x: auto; }} .tables-row {{ flex-direction: column; align-items: center; }} }}
    </style>
</head>
<body>
    ...
    <script>
        function openTourney(e,t){{var a=document.getElementsByClassName("tabcontent");for(i=0;i<a.length;i++)a[i].style.display="none";var n=document.getElementsByClassName("tablinks");for(i=0;i<n.length;i++)n[i].classList.remove("active");document.getElementById(t).style.display="block",e.currentTarget.classList.add("active")}}
        function toggleView(e){{var t=e.closest(".tabcontent"),a=t.querySelector(".main-draw-view"),n=t.querySelector(".qual-view"),s=t.querySelector(".changes-view"),r=t.querySelector(".sub-title");if("flex"===s.style.display){{s.style.display="none";t.querySelector(".changes-btn").style.display="block";t.querySelector(".back-to-qual-btn").style.display="none"}}var o="none"!==a.style.display;a.style.display=o?"none":"flex",n.style.display=o?"flex":"none",e.innerText=o?"Switch to Main Draw":"Switch to Qualifying",r.innerText=o?"QUALIFYING ENTRY LIST":"MAIN DRAW ENTRY LIST"}}
        function showChanges(e,t){{var a=document.getElementById(t);a.querySelector(".main-draw-view").style.display="none",a.querySelector(".qual-view").style.display="none",a.querySelector(".changes-view").style.display="flex",a.querySelector(".sub-title").innerText="LIST OF CHANGES",e.style.display="none",a.querySelector(".main-qual-toggle").innerText="Switch to Main Draw",a.querySelector(".back-to-qual-btn").style.display="block"}}
        function showQualFromChanges(e){{var t=e.closest(".tabcontent");t.querySelector(".changes-view").style.display="none",t.querySelector(".qual-view").style.display="flex",t.querySelector(".sub-title").innerText="QUALIFYING ENTRY LIST",e.style.display="none",t.querySelector(".changes-btn").style.display="block",t.querySelector(".main-qual-toggle").innerText="Switch to Main Draw"}}
    </script>
</body>
</html>"""
