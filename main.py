...
def scrape_tournament(url, tab_label, tid):
    ...
    if not main_df.empty: track_changes(tid, "Main Draw", main_df['Player'].tolist(), full_name)
    if not qual_df.empty: track_changes(tid, "Qualifying", qual_df['Player'].tolist(), full_name)
...
def main():
        ...
        <style>
            ...
            * {{ -webkit-tap-highlight-color: transparent; box-sizing: border-box; }}
            ...
            @media (max-width: 768px) {{
                ...
                .sidebar {{ width: 100%; height: auto; display: flex; overflow-x: auto; white-space: nowrap; padding: 10px 8px; gap: 8px; }}
                .tablinks {{ width: auto; display: inline-block; padding: 8px 16px; border: 1px solid rgba(255, 255, 255, 0.4); border-radius: 20px; font-size: 0.75rem; }}
                .main-content {{ padding: 15px 10px 100px; }}
                .top-row {{ flex-direction: column; height: auto; gap: 15px; }}
    ...
    for tid, entries in history.items():
        current_run_changes = [e['change'] for e in entries if e['date'] == today]
        if current_run_changes:
            clean_tid = tid.replace("_", " ")
            clean_changes = [re.sub('<[^<]+?>', '', c) for c in current_run_changes]
            all_new_changes.append(f"Tournament: {clean_tid}\n" + "\n".join(f"- {c}" for c in clean_changes))
