...
def scrape_tournament(url, tab_label):
    ...
    def generate_split_tables(df):
        if len(df) > 25:
            ...
            return (f'<div class="table-column">{df1.to_html(index=False, classes="entry-table", border=0)}</div>'
                    f'<div class="table-column">{df2.to_html(index=False, classes="entry-table", border=0)}</div>')
        return f'<div class="table-column">{df.to_html(index=False, classes="entry-table", border=0)}</div>'
...
def main():
    ...
    <head>
        ...
        <style>
            ...
            /* UPDATED: Subtitle now uses MontserratExtraBold and same text-shadow */
            .sub-title {{ 
